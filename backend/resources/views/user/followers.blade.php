      ...
      <div class="bg-white shadow border pt-1">
        ...
        <div class="row">
          <div class="col-md-12 d-inline-flex">
            ...
            <div class="d-block user_show_btns">
              <a href="{{ route("user.followers", ["user" => $user, "sort" => "default"]) }}" class="btn btn-light user_show_btn pt-2 read_btn_active" id="follower_btn">
                <small>フォロワー</small>
                <p class="text-center">
        ...
        <div class="drop-down">
          ...
          <div class="dropdown-menu" aria-labelledby="dropdownMenu">
            @foreach($follow_sort as $key => $value)
              <a href="{{ route("user.followers", ["user" => $user ->id ,"sort" => $key]) }}" class="dropdown-item">{{ $value }}</a>
            @endforeach
          </div>
        </div>
      </div>
    ...
    @foreach($followers as $follower_user)
      <div class="col-md-4 mt-2">
        <div class="bg-white p-3 border w-100 h-100">
          <div class="d-flex">
            <a href=" {{ route("user.reads", ["user" => $follower_user -> follower_id, "sort" => "default"]) }}">
              @if($follower_user->image == "noimage.jpg")
                <img src="{{asset('images/noimage.jpg')}}" class="rounded-circle shadow-sm mr-4" width="80px" height="80px">
              @else
                <img src="{{asset('storage/images/'.$follower_user->image)}}" class="rounded-circle shadow-sm mr-4" width="80px" height="80px">
              @endif
            </a>
            <div class="d-flex flex-column">
              <strong class="d-flex flex-row">
              <a href=" {{ route("user.reads", ["user" => $follower_user -> follower_id, "sort" => "default"]) }}" class="text-dark mr-3">
                {{ $follower_user -> name }}
              </a>
              </strong>
              <small class="mt-3">
          ...
          <div class="d-flex mt-5">
            <div class="flex-column w-25 text-center">
              <i class="fas fa-book-reader fa-2x text-dark"></i>
              <p class="text-center">
                {{ $follower_user -> reads_count }}
              </p>
            </div>
            <div class="flex-column w-25 text-center">
              <i class="far fa-plus-square fa-2x text-dark"></i>
              <p class="text-center">
                {{ $follower_user -> unreads_count }}
              </p>
            </div>
            <div class="flex-column w-25 text-center mt-2">
              <small>フォロー</small>
              <p class="text-center">
                {{ $follower_user -> follower_count }}
              </p>
            </div>
            <div class="flex-column w-25 text-center mt-2">
              <small>フォロワー</small>
              <p class="text-center">
                {{ $follower_user -> followed_count }}
              </p>
            </div>
          </div>
        </div>
